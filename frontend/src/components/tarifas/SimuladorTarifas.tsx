...
import { SimuladorContratacion } from './SimuladorContratacion';
import { esContratacion, fmtMXN } from './format';
...
/**
 * Simulador de tarifas. En periódicas calcula el importe por consumo (la administración y la
 * clase son obligatorias: sin ellas el backend agrega todas las administraciones y clases del
 * servicio y el importe no corresponde a ningún contrato real); en contratación cotiza el pago
 * único de un concepto.
 */
export function SimuladorTarifas({ useApi, administraciones, categorias, servicios }: Props) {
  const { toast } = useToast();
  const [seccion, setSeccion] = useState<SeccionTarifa>('PERIODICA');
  ...
  const tiposServicio = useMemo(() => {
    ...
    for (const s of servicios) {
      if (esContratacion(s.seccion)) continue;
      vistos.set(s.tipoServicio, (vistos.get(s.tipoServicio) ?? 0) + s.total);
    }
  ...
  return (
    <div className="space-y-4">
      <SeccionToggle
        value={seccion}
        onChange={(s) => setSeccion(s ?? 'PERIODICA')}
        ariaLabel="Modo del simulador"
      />

      {esContratacion(seccion) ? (
        <SimuladorContratacion
          useApi={useApi}
